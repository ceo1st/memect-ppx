...
from typing import Any, Final, Iterator, Mapping, Sequence, TypedDict, cast
...
class _DetectApi:
    ...
    def detect(self, page: Page, bbox: BBox, *, image: bytes | None = None, name: str = 'wbk') -> _DetectResult:
        """
        获得页面指定区域的表格的单元格信息  
        page:
        bbox: 相对页面的矩形区域，原点为页面左下角
        image: 如果提供了，表示为截图区域的图片内容，目的是多个调用的时候，避免重复截图
        ...
        result = self._api.execute(image, params=params)
        return self._convert_result(page,bbox,image,name,result)

    def batch(self,blocks:Sequence[Block],name: str = 'wbk')->Iterator[Any]:
        #如果提供的是block，截图且转化为相对页面的坐标
        #如果是合并后的
        #yield from self._api.batch(files)
        def get_images()->Iterator[tuple[str,bytes,Any]]:
            cfg = self._models[name]
            params = {
                'models': [cfg['name']]
            }
            #TODO 这里面也可以使用迭代器，这样即使几千个表格也不影响
            for i,block in enumerate(blocks):
                yield (f'{i}',_image_to_bytes(block.page.crop(block.bbox.large)),params)
        
        for block,(_,image,result) in zip(blocks,self._api.batch(get_images())):
            yield self._convert_result(block.page,block.bbox.large,image,name,result)


    def _convert_result(self,page:Page,bbox:BBox,image:bytes,name:str,result:Any)->Any:
        image_size: Final = images.get_size(image)
        scale: Final = bbox.width/image_size[0]
        # 图片坐标的原点为左上角
        image_bbox: Final = bbox.change_origin(
            page.height).scale(image_size[0]/bbox.width)
        m = M(1, 0, 0, 1, 0, 0).translate((image_bbox.x0, image_bbox.y0)).scale(
            scale, scale).scale(1, -1).translate((0, page.height)).to_tuple()
        ...
        vobjs: list[VObject] = []
        for obj in model_result['objects']:
            #obj_bbox = BBox(obj['bbox']).transform(m).large.ensure(page.bbox)
            type_ = obj['type']
            if cfg.get('mapping'):
                type_ = cfg['mapping'].get(type_, type_)

            obj_bbox = BBox(obj['bbox']).transform(m).large.ensure(page.bbox)
            vobj = VObject(page, obj_bbox, type=type_,
                           score=obj['score'], kind='table', model_name=name)
            vobjs.append(vobj)
...
def _get_vobjects22(vobjects: Sequence[VObject], kind: str, types: Sequence[str], scores: float | Mapping[str, float], *, bbox: BBox | None = None, iou_ratio: float = 0.2, iou_types: Sequence[str] | None = None, model_names: Sequence[str] | None = None) -> list['VObject']:
    """获得指定类型的对象，且按分数从高到低排序。
    如果有重叠的，会过滤掉分数低的。

    先根据iou的设置，对互相重叠的进行处理，然后再从中选择指定的类型

    types: 需要哪些类型,空表示所有类型

    scores: float 或者 dict，分数阀值

    bbox: 如果指定了，表示仅仅选择这个区域内的对象

    iou_ratio: 重叠超过这个的，保留分数值最大的,0表示不执行iou

    iou_types: 表示处理重叠的时候需要考虑的类型，None或者为空，表示所有类型
    """

    def iou(vobj: VObject, index: int, vobjs: list[VObject], ratio: float = 0.2) -> VObject:
        # 采用最简单的算法，重叠的就删除，因为重叠的对象已经不多
        if ratio == 0:
            # 不执行
            return vobj
        i = index
        overlapped_objects: list[VObject] = []
        while i < len(vobjs):
            vobj2 = vobjs[i]
            b = vobj.bbox.intersect([vobj2.bbox])
            if b is None or vobj2.bbox.area==0 or vobj.bbox.area==0:
                #TODO 太小的都可以忽略了
                i += 1
                continue
            r = b.area/(vobj.bbox.area+vobj2.bbox.area-b.area)
            if r >= ratio or b.area/vobj2.bbox.area > 0.5:
                # 如果重叠的面积超过比例，或者重叠的面积超过自身的0.5（很小的对象在一个大的对象中，主要是行内数学公式）
                del vobjs[i]
                # 如果需要删除vobj2的重叠的对象
                vobj2 = iou(vobj2, i, vobjs, iou_ratio)
                overlapped_objects.append(vobj2)
            else:
                i += 1

        # 如果没有，可以总是返回同一个对象，因为
        if len(overlapped_objects) == 0 and len(vobj.overlapped_objects) == 0:
            return vobj
        else:
            #TODO 如果有重叠，可能存在vobj1,vobj2,vobj3
            #[--vobj2--]
            #[--vobj3---]
            #vobj1=[vobj2,vobj3]
            #可能vobj3高一点，返回了vobj3，但是实际上vobj1可能更好？
            #这里再比较一下？
            if False and len(overlapped_objects)>0:
                #先获得面积最大的，然后和vobj相比较，如果包含了vobj，且分数相差不大，返回面积大的？
                overlapped_objects.sort(key=lambda vobj:vobj.bbox.area,reverse=True)
                vobj2 = overlapped_objects[0]
                if vobj2.bbox.adjust(dx=5,dy=5).contains([vobj]) and vobj.score-vobj2.score<=0.1:
                    #交换一下？
                    overlapped_objects[0]=vobj
                    vobj2=vobj
                pass
            return vobj.copy(overlapped_objects=overlapped_objects)
    ...
    for vobj in vobjects:
        if vobj.kind != kind:
            continue
        if model_names and vobj.model_name not in model_names:
            continue
        # and (bbox is None or bbox.contains([vobj.bbox])):
        if (not iou_types or vobj.type in iou_types):
            # 这里先不需要考虑bbox
            vobjs.append(vobj)
    # 按分数排序，从高到低排序
    vobjs.sort(key=lambda vobj: vobj.score, reverse=True)
    new_vobjs: list[VObject] = []
    while vobjs:
        vobj = iou(vobjs.pop(0), 0, vobjs, iou_ratio)
        score: float = scores.get(vobj.type) or scores.get('_') or 0.5
        if vobj.score >= score and (not types or vobj.type in types) and (bbox is None or bbox.contains([vobj.bbox])):
            # 如果没有types，表示全部类型
            new_vobjs.append(vobj)
        else:
            # 表示为iou_types中的，不需要，只是用来过滤对象
            pass
    return new_vobjs
...
class Parser:
    ...
    def batch(self,blocks:Sequence[Block])->Iterator[Table|None]:
        """批量执行"""
        #可以批量进行api调用，提高性能
        for block,result in zip(blocks,self._detect_api.batch(blocks,name='wbk')):
            yield self._parse(block,result)

    def parse(self,block:Block) -> Table|None:
        """解析单个表格"""
        result = self._detect_api.detect(
            block.page, block.bbox.large, name='wbk')
        return self._parse(block,result)

    def _parse(self, block: Block,result:Mapping[str,Any]) -> Table | None:
    ...
    def _fix0(self,wtable:WTable,is_row:bool):
        ...
        def get_x(left_cells:Sequence[WCell],right_cells:Sequence[WCell])->float|None:
            ...
            for cell in left_cells:
                ...
                if cb is not None:
                    #left_x.append(cb[x1])
                    left_x.append(cell.full_bbox[x1])
            for cell in right_cells:
                cb = cell.content_bbox
                if cb is not None:
                    #right_x.append(cb[x0])
                    right_x.append(cell.full_bbox[x0])
            ...
            if is_row:
                ...
                if not left_x:
                    ...
                    #上下都有内容
                    #因为行的方式，所以就仅仅一点重叠
                    max_overlapped_height=3
                    if wtable.pages[0].type==PageType.IMAGE:
                        max_overlapped_height=round(4*wtable.pages[0].height/842)
                    a=min(left_x)
                    b=max(right_x)
                    #if debugger.allow('info'):
                    ...
                    #| |
                    #|a
                    if strict and b-a>max_overlapped_height:
                        self._logger.warning(f'行的叠加高度为:{b-a},max={max_overlapped_height}')
                        if debugger.allow('info'):
                            with debugger.group('cells'):
                                for c in left_cells:
                                    debugger.console.print(('top',c.bbox,c.text))
                                for c in right_cells:
                                    debugger.console.print(('bottom',c.bbox,c.text))
                        return None
                    x=(a+b)//2
                ...
                if not left_x:
                    ...
                    if wtable.pages[0].type==PageType.IMAGE:
                        max_overlapped_width=round(10*wtable.pages[0].width/596)
                    a=max(left_x)
                    b=min(right_x)
                    #if debugger.allow('info'):
                    ...
                    if strict and a-b>max_overlapped_width:
                        #可以为负值，表示没有重叠
                        self._logger.warning(f'a={a},b={b},列的叠加长度为:{a-b},max={max_overlapped_width},strict={strict}')
                        if debugger.allow('info'):
                            with debugger.group('cells'):
                                for c in left_cells:
                                    debugger.console.print(('left',c.bbox,c.text))
                                for c in right_cells:
                                    debugger.console.print(('right',c.bbox,c.text))
                        return None
                    x=(a+b)//2
        ...
        def adjust_blank_cells(blank_cells:list[WCell],left_cells:list[WCell],right_cells:list[WCell],prev_cells:list[WCell],next_cells:list[WCell],is_row:bool)->bool:
            """交叉形成的空白单元格和相邻的合并"""
            if not blank_cells:
                return True
            if is_row:
                pending_tasks:list[tuple[WCell,WCell,bool]]=[]
                for c in blank_cells:
                    ok=False
                    #先和前一个合并看看
                    if prev_cells:
                        for c2 in prev_cells:
                            if c2.col_index==c.col_index and c2.col_span==c.col_span and wtable.can_merge(c2.row_index,c.row_index+c.row_span,c.col_index,c.col_index+c.col_span):
                                pending_tasks.append((c,c2,True))
                                ok=True
                                break
                    
                    #不行，再和后一个
                    if not ok and next_cells:
                        for c2 in next_cells:
                            if c2.col_index==c.col_index and c2.col_span==c.col_span and wtable.can_merge(c.row_index,c2.row_index+c2.row_span,c.col_index,c.col_index+c.col_span):
                                pending_tasks.append((c,c2,False))
                                ok=True
                                break
                ...
                if len(pending_tasks)!=len(blank_cells):
                    return False
                
                for c,c2,prev in pending_tasks:
                    if prev:
                        c3=wtable.merge_cells(c2.row_index,c.row_index+c.row_span,c.col_index,c.col_index+c.col_span)
                        assert c3 is not None
                        prev_cells.remove(c2)
                        left_cells.append(c3)
                    else:
                        c3=wtable.merge_cells(c.row_index,c2.row_index+c2.row_span,c.col_index,c.col_index+c.col_span)
                        assert c3 is not None
                        next_cells.remove(c2)
                        right_cells.append(c3)
                return True
            else:
                pending_tasks:list[tuple[WCell,WCell,bool]]=[]
                for c in blank_cells:
                    ...
                    if prev_cells:
                        for c2 in prev_cells:
                            if c2.row_index==c.row_index and c2.row_span==c.row_span and wtable.can_merge(c2.row_index,c2.row_index+c2.row_span,c2.col_index,c.col_index+c.col_span):
                                pending_tasks.append((c,c2,True))
                                ok=True
                                break
                        else:
                    ...
                    if not ok and next_cells:
                        for c2 in next_cells:
                            if c2.row_index==c.row_index and c2.row_span==c.row_span and wtable.can_merge(c2.row_index,c2.row_index+c2.row_span,c.col_index,c2.col_index+c2.col_span) :
                                pending_tasks.append((c,c2,False))
                                ok=True
                                break
                    if not ok:
                ...
                for c,c2,prev in pending_tasks:
                    if prev:
                        #[c2][c] => [c2,c]
                        c3=wtable.merge_cells(c2.row_index,c2.row_index+c2.row_span,c2.col_index,c.col_index+c.col_span)
                        assert c3 is not None
                        prev_cells.remove(c2)
                        left_cells.append(c3)
                    else:
                        #[c][c2] => [c,c2]
                        c3=wtable.merge_cells(c2.row_index,c2.row_index+c2.row_span,c.col_index,c2.col_index+c2.col_span)
                        assert c3 is not None
                        next_cells.remove(c2)
                        right_cells.append(c3)
                return True
            ...
            for i in range(n):
                ...
                for cell in cells:
                    if is_row:
                        ...
                        if cell.row_span==1:
                            #TODO 忽略空白的行吗？
                            if len(cell.objects)>0 or not ignore_blank_cell:
                ...
                if is_row:
                    next_cells = [c for c in wtable.cells if c.row_index==i+1]
                    prev_cells = [c for c in wtable.cells if c.row_index+c.row_span==i]
                else:
                    next_cells = [c for c in wtable.cells if c.col_index==i+1]
                    #如果忽略了空白的cell，这里
                    prev_cells = [c for c in wtable.cells if c.col_index+c.col_span==i]
                ...
                if debugger.allow('gui'):
                    ...
                    wtable.show('prev_cells',bboxes=prev_cells)
                    wtable.show('next cells',bboxes=next_cells)
                    wtable.show('blank_cells',bboxes=blank_cells)
                ...
                if True:
                    if next_cells:
                        ...
                        if x is None and debugger.allow('info'):
                            debugger.print(f'需要调整的座标溢出了next,is_row={is_row},cb={cb},x={old_x}')
                 
                        if x is None:
                            continue

                    if prev_cells:
                        #cb = (prev_cells[0].content_bbox or prev_cells[0].bbox)
                        cb = prev_cells[0].full_bbox
                        old_x=x
                        if is_row:
                            if x>cb.y0:
                                x=None
                            ...
                            if x<cb.x1:
                                x=None
                        if x is None and debugger.allow('info'):
                            debugger.print(f'需要调整的座标溢出了prev,is_row={is_row},cb={cb},x={old_x}')
                ...
                if not adjust_blank_cells(blank_cells,left_cells,right_cells,prev_cells,next_cells,is_row):
                    if debugger.allow('info'):
                        debugger.print('不是所有的交叉空白单元格都能够合并')
                    continue
...
class _PDFParser:
    ...
    def parse(self, block: Block, *, vobjects: Sequence[VObject] | None = None) -> list[WCell]:
        ...
        #TODO 如果是来自ocr的，不需要使用这个，因为很容易把相邻的单元格的文本合并
        #如果是来自pdf的，应该需要添加书写顺序的检查，这样更好一些
        case2(spans)
        # 然后划分数字列，这些列的去掉
        # 剩余的，再根据大一些间距合并
        # self._split1(spans)
